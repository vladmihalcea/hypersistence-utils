...
The main advantage of this project is that it supports a broad range of Hibernate versions, spanning from Hibernate ORM **7.4**, **7.3**, **7.2**, **7.1**, **7.0**, to **6.6**, **6.5**, **6.4**, **6.3**, **6.2**, **6.1**, **6.0**, **5.6**, **5.5**, **5.4**, **5.3**, **5.2**, **5.1**, and **5.0**.

### Installation Guide

Depending on the Hibernate version you are using, you need to add the following dependency:

#### Hibernate 7.4 and 7.3
...
#### Hibernate 6.1 and 6.0 (Commercial support only)
...
Immutable JSON POJOs do not need setters, but Jackson must know how to construct them.
For example, annotate the constructor with `@JsonCreator` and its parameters with `@JsonProperty`.
An all-arguments constructor generated by Lombok does not, by itself, supply this Jackson configuration.
If deserialization fails, inspect the original Jackson exception for missing creators, type discriminators, or incompatible property types.
