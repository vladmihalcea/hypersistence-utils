...
public class BatchSequenceGenerator implements BulkInsertionCapableIdentifierGenerator, PersistentIdentifierGenerator, Configurable {
    ...
    public void configure(Type type, Properties params, Dialect dialect)
        ...
        String sequenceName = determineSequenceName(params, dialect);
        ...
        this.databaseStructure = this.buildDatabaseStructure(type, sequenceName, dialect);
    }
    ...
    private SequenceStructure buildDatabaseStructure(Type type, String sequenceName, Dialect dialect) {
        return new SequenceStructure(dialect, sequenceName, 1, 1, type.getReturnedClass());
    }

    private static String determineSequenceName(Properties params, Dialect dialect) {
        ObjectNameNormalizer normalizer = (ObjectNameNormalizer) params.get(IDENTIFIER_NORMALIZER);
        String sequenceName = normalizer.normalizeIdentifierQuoting(
            ConfigurationHelper.getString(SEQUENCE_PARAM, params));
    ...
    private IdentifierPool replenishIdentifierPool(SessionImplementor session) throws HibernateException {
        List<Serializable> identifiers = new ArrayList<Serializable>(this.fetchSize);
        JdbcCoordinator coordinator = session.getTransactionCoordinator().getJdbcCoordinator();
        ...
        try {
            ...
            try {
                ...
                coordinator.release(resultSet, statement);
            }
        } catch (SQLException e) {
            throw session.getFactory().getSQLExceptionHelper()
                .convert(e, "could not get next sequence value", this.select);
        } finally {
            coordinator.release(statement);
        }
        if (identifiers.size() != this.fetchSize) {
            throw new IdentifierGenerationException(
                "expected " + this.fetchSize + " values from " + this.getSequenceName() + " but got " + identifiers.size());
        }
    ...
    static final class IdentifierPool {
        ...
        static IdentifierPool empty() {
            return new IdentifierPool(Collections.<Serializable>emptyList());
        }
